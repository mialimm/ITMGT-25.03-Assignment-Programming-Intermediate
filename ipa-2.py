...
caesar_cipher("HAPPY BIRTHDAY", 3)
...
def shift_by_letter(letter, letter_shift):
    ...
    try:
        ...
        return "The letter must be a single uppercase English letter or a space. Additionally, letter_shift must be a single uppercase English letter."
...
def scytale_cipher(message, shift):
    ...
    # Stay within the function. Only use the parameters as input. The function should return your answer.
    try:
        if len(message)>=shift and shift>0:
            if len(message) % shift != 0:
                message += "_" * (shift - len(message) % shift)
            new_msg = ""
            for i in range(len(message)):
                encoded_index = (i // shift)+(len(message)//shift)*(i % shift)
                x = message[encoded_index]
                new_msg += x
        return new_msg
    except:
        return "The message must be a string of uppercase English letters and underscoes. Shift must be a positive integer not exceeding the length of the message."
...
def scytale_decipher(message, shift):
    ...
    try:
        if len(message)>=shift and shift>0:
            raw_msg = ""
            for i in range(len(message)):
                decoded_index = (i%(len(message)//shift))*shift+(i//(len(message)//shift))
                y = message[decoded_index]
                raw_msg += y
        return raw_msg
    except:
        return "The message must be a string of uppercase English letters and underscores. Shift should be a positive integer not exceeding the length of the message."
